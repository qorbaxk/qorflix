...
const Movies: React.FC = () => {
  ...
  const getAllMovies = async () => {
    ...
    const allTimeMovieApi = api.get(
      `/discover/movie?api_key=${API_KEY}&language=ko-KR&sort_by=${sorting}&primary_release_date.gte=${
        years.minYear
      }-01-01&primary_release_date.lte=${
        years.maxYear
      }-12-31&with_genres=${choices.join(
        ',',
  ...
  useEffect(() => {
    getAllMovies()
    window.scrollTo(0, 0)
  }, [page, keyword, sorting, years.maxYear, years.minYear, choices])
